...
class AuthenticationTest(TestCase):
    ...
    def test_login_wrong_user(self) -> None:
        response = self.client.post(
            ...
            data={"username": "username", "password": 00000000},
        )
    ...
    def test_login_user(self) -> None:
        response = self.client.post(
            path=reverse("auth:login"),
            data={"username": "ali", "password": "TEST!@#$"},
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            list(json.loads(response.content).keys()), ["refresh", "access"]
        )
    ...
    def test_register_user(self) -> None:
        ...
        self.assertEqual(
            list(json.loads(response.content).keys()), ["username", "password"]
        )
